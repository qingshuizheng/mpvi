                 ...
                 (setq load-opts
                       (mapconcat
                        ...
                        (append `((start . ,beg))
                                ...
                                `((force-media-title . ,(format "hello-%s" (float-time))))
                                load-opts)
                        ","))
                 (setq play-cmd
                       `(batch
...
If ALIST exists, only refresh the property data in it."
  (when (and (null mpvi-control--refreshing) (> (- (float-time) (or mpvi-control--last-update-time 0)) 0.1))
...
(cl-defmethod mpvi-event :around (type data)
  ...
  (ignore-errors
    ...
    (when (eq type 'start-file)
      (mapc #'emms-player-mpv-observe-property '(speed volume ab-loop-a ab-loop-b mute video pause ontop)))
    (when (eq type 'property-change)
      (pcase (intern (alist-get 'name data))
        ('mute (setq mpvi-mpv-mute-p (alist-get 'data data)))
        ('ontop (setq mpvi-mpv-ontop-p (alist-get 'data data)))))
